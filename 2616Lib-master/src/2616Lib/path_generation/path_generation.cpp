...
namespace Path_Generation {
    ...
    if (curr == 0) {
      ...
      if (path.size() == 2) {
        ...
        if (curr + 3 == path.size()) {
          ...
          Point vD = Point(sin(path.at(curr + 2).angle), cos(path.at(curr + 2).angle)) * magnitude_vD;
        } else {
          ...
          Point_vD = Point(sin(path.at(curr + 2).angle), cos(path.at(curr + 2).angle)) * magnitude_vD;
        }
  ...
  double trapezoidal_motion_profile(double distance, double total_distance, double max_velocity, double max_acceleration) {
    ...
    //Distance is too short for robot to reach `max_velocity`
    if (total_distance <= 2 * dist_to_accel) {
      plateau_dist = 0;
      dist_to_accel = total_distance / 2.0;

      //Recalculate `max_velocity` to appropriate value
